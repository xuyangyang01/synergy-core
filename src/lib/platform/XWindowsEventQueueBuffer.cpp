...
XWindowsEventQueueBuffer::waitForEvent (double dtimeout) {
    ...
    if (dtimeout < 0.0) {
        ...
        timeout.tv_usec =
            static_cast<int> (1.0e+6 * (dtimeout - timeout.tv_sec));
        timeoutPtr = &timeout;
    }
...
#define TIMEOUT_DELAY 25
    ...
    while (((dtimeout < 0.0) || (remaining > 0)) && QLength (m_display) == 0 &&
           retval == 0) {
#if HAVE_POLL
        retval = poll (pfds, 2, TIMEOUT_DELAY); // 16ms = 60hz, but we make it >
                                                // to play nicely with the cpu
...
EventQueueTimer*
XWindowsEventQueueBuffer::newTimer (double /*duration*/,
                                    bool /*oneShot*/) const {
